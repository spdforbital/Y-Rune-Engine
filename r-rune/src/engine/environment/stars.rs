...
impl StarsRenderer {
    ...
    pub unsafe fn record_commands(
        ...
        let night = (1.0 - (sun_intensity / 40.0)).clamp(0.0, 1.0);
        let intensity = night * night;
        if intensity <= 0.01 {
            return;
        }
...
fn create_pipeline(
    ...
    let push_constant = vk::PushConstantRange::default()
        ...
        .offset(0)
        .size(size_of::<StarsPushConstants>() as u32);
